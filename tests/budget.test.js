...
describe("Validare tranzactie", () => {
    ...
    test("Accepta descriere cu exact 3 caractere", () => {
        expect(validateTransaction({...validateTransactionTest, description: "abc"}).valid).toBe(true);
    });
    test("Accepta descriere cu exact 60 de caractere", () => {
        const description = "a".repeat(60);
        expect(validateTransaction({...validateTransactionTest, description}).valid).toBe(true);
    });
    test("Respinge descrieri prea scurte", () => {
        expect(validateTransaction({...validateTransactionTest, description: "ab"}).valid).toBe(false);
    });
    test("Respinge descrieri mai lungi de 60 de caractere", () => {
        const description = "a".repeat(61);
        expect(validateTransaction({...validateTransactionTest, description}).valid).toBe(false);
    });
    test("Accepta suma minima pozitiva 0.01", () => {
        expect(validateTransaction({...validateTransactionTest, amount: "0.01"}).valid).toBe(true);
    });
    test("Respinge suma minima 0", () => {
        expect(validateTransaction({...validateTransactionTest, amount: "0"}).valid).toBe(false);
    });
    test("Respinge sume mai mari de limita maxima 1.000.000", () => {
        expect(validateTransaction({...validateTransactionTest, amount: "1000000.01"}).valid).toBe(false);
    });
    // Equivalence partitioning / Input Validation
    test("Respinge sume negative", () => {
        expect(validateTransaction({...validateTransactionTest, amount: "-20"}).valid).toBe(false);
    });
    test("Respinge suma goala", () => {
        expect(validateTransaction({...validateTransactionTest, amount: ""}).valid).toBe(false);
    });
    test("Respinge cateogrii invalide", () => {
        expect(validateTransaction({...validateTransactionTest, category: "car"}).valid).toBe(false);
    });
    test("Respinge tipuri invalide", () => {
        expect(validateTransaction({...validateTransactionTest, type: "gift"}).valid).toBe(false);
    });
    test("Respinge date invalide", () => {
        expect(validateTransaction({...validateTransactionTest, date: ""}).valid).toBe(false);
    });
    test("Respinge date din viitor", () => {
        expect(validateTransaction({...validateTransactionTest, date: "2077-05-03"}).valid).toBe(false);
    });
    ...
    test("Elimina spatiile inutile din descriere", () => {
        const result = validateTransaction({...validateTransactionTest, description: "    Chirie   "});
        expect(result.transaction.description).toBe("Chirie");
    });
});
...
describe("Filtrare tranzactii", () => {
  ...
  test("Filtreaza dupa text", () => {
    expect(filterTransactions(transactions, {search: "cafea"})).toHaveLength(1);
  });
  test("Filtre combinate", () => {
    expect(filterTransactions(transactions, {search: "met", type: "expense", category: "transport"})).toHaveLength(1);
  });
  // Functional testing / Equivalence partitioning
  test("Filtreaza text cu majuscule", () => {
    expect(filterTransactions(transactions, {search: "SALARIU"})).toHaveLength(1);
  });
  test("Filtreaza dupa tip", () => {
    expect(filterTransactions(transactions, {type: "expense"})).toHaveLength(2);
  });
  test("Filtreaza dupa categorie", () => {
    expect(filterTransactions(transactions, {category: "salary"})).toHaveLength(1);
  });
});
